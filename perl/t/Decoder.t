...
my $decoder = Zebra::Decoder->new();
isa_ok($decoder, 'Zebra::Decoder', 'decoder');
...
is($sym, Zebra::Symbol::NONE, 'enum/enum compare');
...
$decoder->set_handler(sub {
    ...
    $handler_type = $type
      if(!$handler_type or $type > Zebra::Symbol::PARTIAL);
...
$decoder->reset();
is($decoder->get_color(), Zebra::SPACE, 'reset color');
...
foreach my $width (split(/ */, $encoded)) {
    ...
    if($tmp > Zebra::Symbol::PARTIAL) {
        $sym = ($sym == Zebra::Symbol::NONE) ? $tmp : -1;
    }
}
is($sym, Zebra::Symbol::EAN13, 'EAN-13 type');
...
is($decoder->get_color(), Zebra::BAR, 'post-scan color');

#########################

is($handler_type, Zebra::Symbol::EAN13, 'handler type');
