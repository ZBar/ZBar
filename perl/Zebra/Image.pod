...
Zebra::Image - image object to scan for bar codes
...
Zebra::Image is used to pass images to the bar code scanner.  It wraps
raw image data with the meta-data required to interpret it (size,
pixel format, etc)
...
Create a new Zebra::Image object.  The size, pixel format and data
...
Return a list of scanned Zebra::Symbol results attached to this image.

=item convert(I<format>)

Return a new Zebra::Image object converted to the indicated fourcc
format.  Returns C<undef> if the conversion is not supported.
...
Zebra, Zebra::Image, Zebra::Symbol, zebraimg(1), zebracam(1)
