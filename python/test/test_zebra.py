...
class TestZebraFunctions(ut.TestCase):
    ...
    def test_exceptions(self):
        self.assert_(isinstance(zebra.Exception, type))
        self.assert_(issubclass(zebra.InternalError, zebra.Exception))
        self.assert_(issubclass(zebra.UnsupportedError, zebra.Exception))
        self.assert_(issubclass(zebra.InvalidRequestError, zebra.Exception))
        self.assert_(issubclass(zebra.SystemError, zebra.Exception))
        self.assert_(issubclass(zebra.LockingError, zebra.Exception))
        self.assert_(issubclass(zebra.BusyError, zebra.Exception))
        self.assert_(issubclass(zebra.X11DisplayError, zebra.Exception))
        self.assert_(issubclass(zebra.X11ProtocolError, zebra.Exception))
        self.assert_(issubclass(zebra.WindowClosed, zebra.Exception))

class TestScanner(ut.TestCase):
    def setUp(self):
        self.scn = zebra.Scanner()

    def tearDown(self):
        del(self.scn)

    def test_type(self):
        self.assert_(isinstance(self.scn, zebra.Scanner))
        ...
        self.assertRaises(AttributeError, set_color, zebra.BAR)
...
class TestDecoder(ut.TestCase):
    def setUp(self):
        self.dcode = zebra.Decoder()

    def tearDown(self):
        del(self.dcode)

    def test_type(self):
        self.assert_(isinstance(self.dcode, zebra.Decoder))
        ...
        def set_type(typ):
            self.dcode.type = typ
        self.assertRaises(AttributeError, set_type, zebra.Symbol.CODE128)
    ...
    def test_width(self):
        ...
        self.assert_(sym is zebra.Symbol.NONE)
    ...
    def test_reset(self):
        self.assert_(self.dcode.color is zebra.SPACE)
    ...
    def test_decode(self):
        ...
        def handler(dcode, closure):
            ...
            if dcode.type > zebra.Symbol.PARTIAL:
        ...
        for (i, width) in enumerate(encoded_widths):
            ...
            if i < len(encoded_widths) - 1:
                self.assert_(sym is zebra.Symbol.NONE or
                             sym is zebra.Symbol.PARTIAL)
            else:
                self.assert_(sym is zebra.Symbol.EAN13)

        self.assertEqual(self.dcode.data, '6268964977804')
        self.assert_(self.dcode.color is zebra.BAR)
        self.assert_(sym is zebra.Symbol.EAN13)
        self.assert_(inline_sym[0] is zebra.Symbol.EAN13)
...
class TestImage(ut.TestCase):
    def setUp(self):
        self.image = zebra.Image(123, 456, 'Y800')

    def tearDown(self):
        del(self.image)

    def test_type(self):
        self.assert_(isinstance(self.image, zebra.Image))
    ...
    def test_new(self):
        ...
        image = zebra.Image()
        self.assert_(isinstance(image, zebra.Image))
...
class TestImageScanner(ut.TestCase):
    def setUp(self):
        self.scn = zebra.ImageScanner()

    def tearDown(self):
        del(self.scn)

    def test_type(self):
        self.assert_(isinstance(self.scn, zebra.ImageScanner))
...
class TestImageScan(ut.TestCase):
    def setUp(self):
        self.scn = zebra.ImageScanner()
        self.image = zebra.Image(size[0], size[1], 'Y800', data)
    ...
    def test_scan(self):
        ...
        i = iter(self.image)
        self.assert_(isinstance(i, zebra.SymbolIter))
        sym = i.next()
        self.assertRaises(StopIteration, i.next)

        # this is the only way to obtain a Symbol,
        # so test Symbol here
        self.assert_(isinstance(sym, zebra.Symbol))
        self.assert_(sym.type is zebra.Symbol.EAN13)
...
class TestProcessor(ut.TestCase):
    def setUp(self):
        self.proc = zebra.Processor()

    def tearDown(self):
        del(self.proc)

    def test_type(self):
        self.assert_(isinstance(self.proc, zebra.Processor))
    ...
    def test_processing(self):
        ...
        def data_handler(proc, image, closure):
            ...
            symiter = iter(image)
            self.assert_(isinstance(symiter, zebra.SymbolIter))

            symbols = tuple(image)
            self.assertEqual(len(symbols), 1)
            for symbol in symbols:
                self.assert_(isinstance(symbol, zebra.Symbol))
                self.assert_(symbol.type is zebra.Symbol.EAN13)
