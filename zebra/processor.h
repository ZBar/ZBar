...
typedef int (poll_handler_t)(zebra_processor_t*, int);
...
struct zebra_processor_s {
    ...
    zebra_video_t *video;               /* input video device abstraction */
    zebra_window_t *window;             /* output window abstraction */
    zebra_image_scanner_t *scanner;     /* barcode scanner */
...
static inline int add_poll (zebra_processor_t *proc,
                            int fd,
...
static inline int remove_poll (zebra_processor_t *proc,
                               int fd)
{
    int i;
...
static inline void emit (zebra_processor_t *proc,
                         unsigned mask)
{
    proc->events |= mask;
#ifdef HAVE_LIBPTHREAD
