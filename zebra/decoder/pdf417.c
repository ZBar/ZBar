...
static inline signed short pdf417_decode8 (zebra_decoder_t *dcode)
{
    ...
    zassert(clst >= 0 && clst < 9, -1, "dir=%x sig=%lx k=%x %s\n",
            ...
            _zebra_decoder_buf_dump(dcode->buf, dcode->pdf417.character));
...
static inline signed char pdf417_decode_start(zebra_decoder_t *dcode)
{
    unsigned s = dcode->pdf417.s8;
    if(s < 8)
        return(0);
    ...
    int ex = (get_color(dcode) == ZEBRA_SPACE) ? 2 : 6;
    if(ei != ex)
        return(0);
    ...
    ex = (get_color(dcode) == ZEBRA_SPACE) ? 0 : 2;
    if(ei != ex)
        return(0);
    ...
    ex = (get_color(dcode) == ZEBRA_SPACE) ? 7 : 1;
    if(ei != ex)
        return(0);

    ei = decode_e(get_width(dcode, 7) + get_width(dcode, 8), s, 17);
    ex = (get_color(dcode) == ZEBRA_SPACE) ? 8 : 1;
...
zebra_symbol_type_t _zebra_decode_pdf417 (zebra_decoder_t *dcode)
{
    ...
    if(get_color(dcode) != dcode417->direction) {
        ...
        zassert(get_color(dcode) == dcode417->direction, ZEBRA_NONE,
                "color=%x dir=%x char=%d elem=0 %s\n",
                get_color(dcode), dcode417->direction, c,
                _zebra_decoder_buf_dump(dcode->buf, c));
    }
